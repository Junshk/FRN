...
class ResidualGroup(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, act, res_scale, n_resblocks, n_resgroups, px):
        super(ResidualGroup, self).__init__()
        modules_body = []
        if len(n_resgroups) ==0:	
            modules_body = [
            RCAB(
                conv, n_feat, kernel_size, reduction, bias=True, bn=False, act=nn.ReLU(True), res_scale=1, px=px) \
            for _ in range(n_resblocks)]
        else:
            modules_body = [
            ResidualGroup(conv, n_feat, kernel_size, reduction, act, res_scale, n_resblocks, n_resgroups[1:], px=px)\
...
class FRN_UPDOWN(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        modules_body = [
            ResidualGroup(
                conv, n_feats, kernel_size, reduction, act=act, res_scale=args.res_scale, n_resblocks=n_resblocks, n_resgroups=n_resgroups[1:], px=args.px) \
            for _ in range(n_resgroups[0])]
